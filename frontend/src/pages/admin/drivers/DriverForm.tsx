...
export function DriverForm() {
  ...
  const [form, setForm] = useState({
    ...
    // Employment
    yearsOfExperience: '',
    vehicleTypesComfortable: [] as string[],
  ...
  const fetchDriverUsers = useCallback(async (query: string) => {
    ...
    try {
      ...
      const res = (await api.get('/lookup/driver-users', { params })) as unknown as {
        _id: string;
        name: string;
        phone: string;
      }[];
      ...
      api
        ...
        .then((res: unknown) => {
          ...
          setForm((prev) => ({
            ...
            permanentAddress:
              (d.permanentAddress as typeof prev.permanentAddress) || prev.permanentAddress,
            emergencyContact:
              (d.emergencyContact as typeof prev.emergencyContact) || prev.emergencyContact,
  ...
  const toggleArrayItem = useCallback(
    (field: 'vehicleTypesComfortable' | 'languagesSpoken', value: string) => {
      setForm((prev) => ({
        ...prev,
      ...
      >
        ...
        {tab === 'personal' && (
          <div className="space-y-4">
            ...
            <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wider pt-2">
              Current Address
            </p>
            <Input
              label="Street"
              value={form.currentAddress.street}
              onChange={(e) => updateNested('currentAddress', 'street', e.target.value)}
            ...
            <div className="grid grid-cols-3 gap-4">
              <Input
                ...
                onChange={(e) => updateNested('currentAddress', 'city', e.target.value)}
                placeholder="Indore"
              />
              <Input
                label="State"
                value={form.currentAddress.state}
                onChange={(e) => updateNested('currentAddress', 'state', e.target.value)}
                placeholder="Madhya Pradesh"
              />
              <Input
                label="Pincode"
                value={form.currentAddress.pincode}
                onChange={(e) => updateNested('currentAddress', 'pincode', e.target.value)}
                placeholder="452001"
              />
            </div>
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateNested('permanentAddress', 'pincode', e.target.value)}
                  />
                </div>
              </>
            )}
        ...
        {tab === 'documents' && (
          <div className="space-y-4">
            ...
            <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wider pt-4">
              Aadhaar Card
            </p>
            <Input
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Input
                ...
                onChange={(e) => updateNested('bankDetails', 'ifscCode', e.target.value)}
            ...
            <Input
              ...
              onChange={(e) => updateNested('bankDetails', 'upiId', e.target.value)}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Select
                ...
                value={form.availableShift}
                onChange={(v) => updateField('availableShift', v)}
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-2">
                Vehicle Types Comfortable With
              </label>
              <div className="flex flex-wrap gap-2">
                {VEHICLE_TYPES.map((vt) => (
                  <button
                    key={vt.value}
                    type="button"
                    onClick={() => toggleArrayItem('vehicleTypesComfortable', vt.value)}
                    className={cn(
              ...
              <div className="flex flex-wrap gap-2">
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.value}
                    type="button"
                    onClick={() => toggleArrayItem('languagesSpoken', lang.value)}
                    className={cn(
                      'px-3 py-1.5 rounded-full text-xs font-medium border transition-all',
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={form.knowsLocalRoutes}
                onChange={(e) => updateField('knowsLocalRoutes', e.target.checked)}
                className="w-4 h-4 rounded border-neutral-300 text-primary-600"
              />
            ...
            <div className="grid grid-cols-2 gap-4">
              <Input
                ...
                onChange={(e) => updateNested('reference', 'name', e.target.value)}
                placeholder="Reference person"
              />
              <Input
                label="Reference Phone"
                value={form.reference.phone}
                onChange={(e) => updateNested('reference', 'phone', e.target.value)}
                placeholder="9876543210"
              />
            </div>
          ...
          <div className="flex gap-2">
            {TABS.indexOf(TABS.find((t) => t.id === tab)!) > 0 && (
              <Button
                variant="outline"
                type="button"
                onClick={() => setTab(TABS[TABS.indexOf(TABS.find((t) => t.id === tab)!) - 1].id)}
              >
                Previous
              </Button>
            )}
